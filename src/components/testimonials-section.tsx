...
function TestimonialCard({ item }: { item: TestimonialItem }) {
  ...
  return (
    <li className="group relative isolate overflow-hidden rounded-2xl border border-line-soft bg-bg-surface p-4 shadow-soft transition-all duration-300 hover:border-line-strong hover:shadow-card">
      <span
        aria-hidden
      ...
      <div className="flex items-center justify-between">
        {rating > 0 ? (
          ...
          <span aria-hidden className="h-3 w-3 rounded-full bg-brand-accent shadow-[0_0_8px_rgba(47,107,255,0.65)]" />
        )}
    ...
    >
      {Array.from({ length: total }, (_, i) => (
        <svg
          ...
          viewBox="0 0 24 24"
          className={
            i < filled ? 'h-3.5 w-3.5 text-gold' : 'h-3.5 w-3.5 text-line-strong'
          }
          fill="currentColor"
          aria-hidden
        >
